...
class iCourses:
    ...
    def login(self, username, password):
        ...
        self.password = base64.b64encode(
            AES.new(self.aeskey, AES.MODE_ECB).encrypt(
                pkcs7padding(password.encode("utf-8"))
            )
        )
        #
        ...
        try:
            data = json.loads(
                self.s.post(url=get_url, headers=headers, verify=False).text
            )
        except:
        ...
        self.captcha = ocr.classification(
            base64.b64decode(captcha[captcha.find(",") + 1 :])
        )
        ...
        response = json.loads(
            self.s.post(url=login_url, headers=headers, data=payload, verify=False).text
        )
    ...
    def setbatchId(self, idx):
        ...
        d = json.loads(
            self.s.post(url=url, headers=headers, data=payload, verify=False).text
        )
        if 200 != d["code"]:
    ...
    def del_lesson(self, ClassType, ClassId, SecretVal):
        ...
        headers = {
            "Host": "icourses.jlu.edu.cn",
            "Origin": "https://icourses.jlu.edu.cn",
            "Referer": "https://icourses.jlu.edu.cn/xsxk/elective/grablessons?batchId="
            + self.batchid,
            "Authorization": self.token,
        ...
        response = json.loads(
            self.s.post(url=post_url, headers=headers, data=payload, verify=False).text
        )
    ...
    def get_select(self):
        ...
        response = json.loads(
            self.s.post(url=post_url, headers=headers, verify=False).text
        )
        if response["code"] == 200:
    ...
    def find(self, key):
        ...
        response = json.loads(
            self.s.post(url=post_url, headers=headers, json=payload, verify=False).text
        )
    ...
    def PrintSelect(self):
        ...
        if self.select != None:
            for item in self.select:
                print(
                    "Teacther: %-10sName: %-20s Id: %-30s"
                    % (item["SKJS"], item["KCM"], item["JXBID"])
                )
    ...
    def PrintFavorite(self):
        ...
        if self.favorite != None:
            for item in self.favorite:
                print(
                    "Teacther: %-10sName: %-20s Id: %-30sclazzType: %-10s"
                    % (
                        item["SKJS"],
                        item["KCM"],
                        item["JXBID"],
                        item["teachingClassType"],
                    )
                )
        ...
        if self.favorite != None:
            for item in self.favorite:
                self.select_favorite(
                    item["teachingClassType"], item["JXBID"], item["secretVal"]
                )
        ...
        if None != self.favorite:
            ...
            for item in self.favorite:
                ...
                for i in range(WorkThreadCount):
                    thread[key].append(
                        threading.Thread(target=self.workThread, args=args)
                    )
                    thread[key][-1].start()
